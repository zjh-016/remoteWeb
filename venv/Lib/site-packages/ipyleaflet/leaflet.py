...
def tianditu_to_tiles(url, crs):
    proj = 'w'
    zoom_offset = 0

    if crs == projections.EPSG4326:
        proj = 'c'
        zoom_offset = 1

    return TDTLayer(
        url=url,
        subdomains=providers.TianDiTu.get('Subdomains'),
        key=providers.TianDiTu.get('key'),
        proj=proj,
        zoom_offset=zoom_offset,
        base=True
    )

def tianditu_to_basemap(crs):
    tianditu = providers.TianDiTu
    normal = tianditu.get('Normal')
    satellite = tianditu.get('Satellite')

    normalLayer = LayerGroup(
        name='线画地图'
    )
    normalLayer.add_layer(tianditu_to_tiles(normal.get('Map'), crs))
    normalLayer.add_layer(tianditu_to_tiles(normal.get('Annotion'), crs))
    normalLayer.base = True

    satelliteLayer = LayerGroup(
        name='遥感地图'
    )
    satelliteLayer.add_layer(tianditu_to_tiles(satellite.get('Map'), crs))
    satelliteLayer.add_layer(tianditu_to_tiles(satellite.get('Annotion'), crs))
    satelliteLayer.base = True
    satelliteLayer.visible = False

    return (normalLayer, satelliteLayer)
...
class WMSPostLayer(TileLayer):
    ...
    crs: dict, default ipyleaflet.projections.EPSG4326
        Projection used for this WMS service.
    """

    _view_name = Unicode('LeafletPostWMSLayerView').tag(sync=True)
    _model_name = Unicode('LeafletPostWMSLayerModel').tag(sync=True)

    # Options
    layers = Unicode().tag(sync=True, o=True)
    styles = Unicode().tag(sync=True, o=True)
    format = Unicode('image/png').tag(sync=True, o=True)
    transparent = Bool(True).tag(sync=True, o=True)
    crs = Dict(default_value=projections.EPSG4326).tag(sync=True)
    uppercase = Bool(True).tag(sync=True, o=True)
    body = Dict(default_value={}).tag(sync=True, o=True)
    # convert request param to camel case, such as: output_format -> outputFormat
    camel_case = Bool(False).tag(sync=True, o=True)
...
class WMSLayer(TileLayer):
    ...
    # convert request param to camel case, such as: output_format -> outputFormat
    camel_case = Bool(True).tag(sync=True, o=True)

class TDTLayer(TileLayer):
    _view_name = Unicode('LeafletTDTLayerView').tag(sync=True)
    _model_name = Unicode('LeafletTDTLayerModel').tag(sync=True)

    #Options
    subdomains = List(default_value=None, allow_none=True).tag(sync=True, o=True)
    key = Unicode().tag(sync=True, o=True)
    proj = Unicode(default_value='w').tag(sync=True, o=True)
...
class Map(DOMWidget, InteractMixin):
    ...
    crs: projection, default projections.EPSG4326
    ...
    def _default_layers(self):
        ...
        return tianditu_to_basemap(self.crs)
