...
def findsource_in_sources(lines, object):
    if ismodule(object):
        return lines, 0
...
def findsource(object):
    ...
    if is_notebook():
        lines = linecache.getlines(getfile_support_notebook(object))
    else:
    ...
    return findsource_in_sources(lines, object)
...
def getfile_support_notebook(object, _old_getfile=getfile):
    ...
    for name, member in getmembers(object):
        if isfunction(member) and object.__qualname__ + '.' + member.__name__ == member.__qualname__:
            return getfile(member)
    else:
